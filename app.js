...
// =============================
//  접속 세션 로그 (기존 기능과 독립 실행)
//  - 사용자 페이지에서 브라우저 탭 세션당 1건만 기록
//  - 로그인 후 같은 접속 로그에 회원 정보를 연결
//  - 실제 IP/User-Agent 저장은 Supabase Edge Function에서 처리
// =============================
(function initEprangAccessTracking() {
  const ACCESS_FUNCTION_NAME = 'access-log';
  const SESSION_ID_KEY = 'ep-access-session-id';
  const VISITOR_ID_KEY = 'ep-visitor-id';
  const LOGGED_KEY = 'ep-access-logged';
  const IDENTIFIED_USER_KEY = 'ep-access-identified-user';
  const IN_FLIGHT_KEY = 'ep-access-in-flight';
  ...
  function shouldSkipAccessTracking() {
  ...
  function getAccessVisitorId() {
    ...
    if (!visitorId) {
      visitorId = createAccessId();
      localStorage.setItem(VISITOR_ID_KEY, visitorId);
    }
    return visitorId;
  }

  async function invokeAccessLog(body) {
    try {
      ...
      // 접속 로그 오류가 로그인·설문·프로젝트 기능에 영향을 주지 않도록 종료합니다.
      console.warn('[이피랑] 접속 로그 저장 실패:', error?.message || error);
      return false;
    }
  }

  async function identifyCurrentAccess(sessionId) {
    try {
      const { data: { session } } = await supabaseClient.auth.getSession();
      const userId = session?.user?.id || '';
      if (!userId || sessionStorage.getItem(IDENTIFIED_USER_KEY) === userId) return;

      const identified = await invokeAccessLog({
        action: 'identify',
      ...
      console.warn('[이피랑] 접속 회원 연결 실패:', error?.message || error);
    }
  }

  async function trackCurrentAccessOnce() {
    if (shouldSkipAccessTracking()) return;
    if (sessionStorage.getItem(IN_FLIGHT_KEY) === '1') return;
    ...
    if (sessionStorage.getItem(LOGGED_KEY) !== '1') {
      ...
      const logged = await invokeAccessLog({
        action: 'start',
        session_id: sessionId,
        visitor_id: getAccessVisitorId(),
        entry_path: window.location.pathname || '/',
        referrer: document.referrer || null,
        screen_width: Number(window.screen?.width || 0) || null,
        screen_height: Number(window.screen?.height || 0) || null,
    ...
    await identifyCurrentAccess(sessionId);
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', trackCurrentAccessOnce, { once: true });
  } else {
    trackCurrentAccessOnce();
  }

  // 같은 페이지에서 로그인 상태가 바뀌는 경우에도 기존 접속 로그와 연결합니다.
